defmodule PwAppWeb.PageLive do
  ...
  def handle_event(
    ...
    new_advice =
      if String.length(results.output) > 0 do
        Feedback.advice(results.analysis, results.type)
      else
    ...
    {:noreply,
     assign(socket,
       ...
       output: if(type == "custom", do: output, else: results.output),
  ...
  def handle_event("generate", %{"output" => output}, socket) do
    {:noreply,
     assign(socket,
       output: output,
